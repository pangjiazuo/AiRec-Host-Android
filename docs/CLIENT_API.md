...
- 人、车、动物运动后记录普通事件；长时间停留仅限人，静止的人也可触发。
- 停留达标更新原事件 ID，保留首次截图和时间。旧历史事件不删除。
- 类别筛选仍可找到该类别的停留事件。动物和车的 `dwell_eligible=false`、`dwell_seconds=0`。
- `motion_detected` 表示本次跟踪曾确认运动，依据连续两次跟踪框变化判断，不额外运行模型。
- `detector.last_candidates` 提供最近一次识别的通道、时间、各类别数量和最高分，方便排查漏检，不含图片。

目标离开超过容忍时间后，再出现可产生新事件。容忍时间至少为采样间隔的 2.5 倍。细微肢体动作和镜头晃动仍需实拍验证。
